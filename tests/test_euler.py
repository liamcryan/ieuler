...
from ieuler import Euler, Submission
...
def test_get_project_by_number_http():
    ...
    assert os.path.isfile(p_1._file()) is False
...
def test_submit_guess():
    ...
    assert p_1.submission is None
    p_1.post(3.14)  # incorrect answer, but we aren't testing that
    assert p_1.logged_in is True
    assert isinstance(p_1.submission, Submission)
    p_1.DIRECTORY = problem_dir


@my_vcr.use_cassette(none=True)
def test_certify_incorrect():
    p_1.post(3.14)  # the wrong answer
    assert p_1.submission.status is False
    p_1.submission.certify()  # doesn't really do anything except warn the user in this case


@my_vcr.use_cassette(none=True)
def test_certify_correct():
    try:
        os.remove(os.path.join(here, 'certificates', 'problem_1_certificate.html'))
    except OSError:
        pass
    p_1.post(233168)  # sorry everyone for the spoiler
    assert p_1.submission.status
    p_1.submission.certify()  # saves the certificate
    assert os.path.isfile(p_1.submission._file())
